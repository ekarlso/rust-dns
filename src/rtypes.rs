...
impl V4Address {
  pub fn new(data: &[u8, ..4]) -> V4Address {
    return V4Address { data: *data }
  }

  pub fn from_str(string: &str) -> Option<V4Address> {
    let split: Vec<Option<u8>> = string.split('.').map(|p| from_str::<u8>(p)).collect();
    let mut data = [0, ..4];
    ...
    for i in range(0, length) {
      if split[i].len() == 0 {
        ...
        if !padded || i > padding_position + padding_length {
          match num::from_str_radix::<u16>(split[i], 10) {
            Some(v) => {
